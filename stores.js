...
(
  ...
  way["shop"~"supermarket|grocery"](around:${radius},${lat},${lng});
);
out center;`;
    ...
    const ebtChains = ['h-e-b', 'heb', 'walmart', 'fiesta', 'aldi', 'kroger', 'randalls', 'whole foods', 'costco'];
    ...
    const stores = data.elements
      .map(el => {
        ...
        const km = slat && slng
          ? Math.sqrt(
              ...
              Math.pow((slng - lng) * 111 * Math.cos(lat * Math.PI / 180), 2)
            )
          : null;
        const dist = km
      ...
      .filter(s => s.name !== 'Grocery Store' || s.addr)
